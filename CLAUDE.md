...
- `git-tree-ops.ts`, `git-commit-write.ts`, `git-branch-ops.ts`, `git-history-ops.ts` — split by responsibility from a single former `git-operations-iso.ts`: tree read/write primitives, commit writing (`createCommit`/`deleteFile`), branch CRUD, and blob/commit/tree reads + history, respectively. `getRepoOptions(ownerKey, repoName)` (hydrate-if-needed + resolve fs/gitdir options) is the shared entry point these all call, defined in `git-repo-storage.ts`.

...
- Every branch-name-shaped field anywhere in the app (`files.ts`, `pull-requests.ts`) must be validated with `safeBranchNameSchema`/`isSafeBranchName` (`git-ref-name.ts`), never a bare `z.string()` — `git.commit`/`git.merge`/`git.deleteBranch` don't validate ref names internally the way `git.branch`/`git.writeRef` do, and the R2 backend derives the target repo by parsing the (already `join()`-normalized) path it's given, so an unvalidated branch name is a cross-tenant path traversal reachable from ordinary web-UI actions (branch delete, PR merge), not just a crafted git push.

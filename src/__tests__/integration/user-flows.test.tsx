...
describe('Integration Tests - Full User Flows', () => {
  it('should handle complete authentication and navigation flow', async () => {
    ...
    const mockLogin = vi.fn().mockImplementation(async (email: string, password: string) => {
    ...
    function LoginPage() {
      ...
      const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
          ...
          ;(window as any).router?.navigate({ to: '/dashboard' })
        } catch (err: any) {
  ...
  it('should handle data fetching, display, and error states', async () => {
    ...
    expect(screen.getByTestId('issue-2')).toHaveTextContent('Feature: Add search')

...
export class R2RefBackend {
   ...
   */
  async readRef(ownerId: string, repoName: string, ref: string): Promise<string> {
    ...
    if (expectedValue !== undefined) {
      try {
        ...
        if (currentValue !== expectedValue) {
          throw new Error(`Ref update conflict: expected ${expectedValue}, found ${currentValue}`)
        }
      } catch (error: any) {
        if (error instanceof GitRefNotFoundError && expectedValue !== null) {
          throw new Error(`Ref update conflict: ref exists but expected it to be null`)
        }
        if (!(error instanceof GitRefNotFoundError) || expectedValue !== null) {
          throw error
        }
   ...
   */
  async deleteRef(ownerId: string, repoName: string, ref: string): Promise<void> {
    const r2Key = getR2Key(ownerId, repoName, ref)
    const cacheKey = `${ownerId}/${repoName}/${ref}`
   ...
   */
  async listRefs(ownerId: string, repoName: string, prefix: string = 'refs/'): Promise<string[]> {

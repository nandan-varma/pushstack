...
// @ts-ignore - importing after mock
...
describe('GitManager - Repository Management', () => {
  ...
  const testGitPath = path.join(process.cwd(), 'data', 'repos', '123', 'test-repo');
  ...
  describe('cloneRepo', () => {
    it('should clone repository from URL', async () => {
      ...
      const result = await GitManager.cloneRepo(sourceUrl, testOwnerId, testRepoName);

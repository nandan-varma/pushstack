...
export async function getCommitDiff(
  ...
  try {
    ...
    const files = changes.filter((c: DiffFile | null): c is DiffFile => c !== null);

    return {
      files,
      totalAdditions: files.reduce((sum: number, f: DiffFile) => sum + f.additions, 0),
      totalDeletions: files.reduce((sum: number, f: DiffFile) => sum + f.deletions, 0),
      totalFiles: files.length,
    };
  } catch (error) {

...
export function getLegacyStorageOwnerKeys(owner: OwnerLike): string[] {
  return [...new Set([
    sanitizeStorageSegment(owner.id),
    'NaN',
  ])]
}
...
export function getRepoStorageCoordinates(repo: RepoLike) {
  ...
  const legacyOwnerKeys = getLegacyStorageOwnerKeys(repo.owner)
    .filter((legacyOwnerKey) => legacyOwnerKey !== ownerKey)
...
export function getRepoGitStorageRoot(ownerKey: string, repoName: string): string {
  return `${getRepoStorageRoot(ownerKey, repoName)}/git`
}

export function getRepoGitStoragePrefix(ownerKey: string, repoName: string): string {
  return `${getRepoGitStorageRoot(ownerKey, repoName)}/`
}

export function getLegacyGitPrefixes(legacyOwnerKeys: string[], repoName: string): string[] {
  const gitPrefixes = legacyOwnerKeys.map((legacyOwnerKey) => getRepoGitStoragePrefix(legacyOwnerKey, repoName))
  const legacyFlatPrefixes = legacyOwnerKeys.map((legacyOwnerKey) => `${getRepoStorageRoot(legacyOwnerKey, repoName)}/`)
  const oldTopLevelPrefixes = legacyOwnerKeys.map((legacyOwnerKey) => `git/${sanitizeStorageSegment(legacyOwnerKey)}/${sanitizeStorageSegment(repoName)}/`)

  return [...new Set([...gitPrefixes, ...legacyFlatPrefixes, ...oldTopLevelPrefixes])]
}

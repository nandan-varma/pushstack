...
export async function handleGitUploadPack(
  ...
  try {
    ...
    throw new Error('Git upload-pack not fully implemented yet. Use alternative git operations.')
  } catch (error) {
...
export async function handleGitReceivePack(
  ...
  try {
    ...
    throw new Error('Git receive-pack not fully implemented yet. Use alternative git operations.')
  } catch (error) {
...
export async function getGitInfoRefs(
  ...
  try {
    ...
    for (const ref of refs) {
      try {
        const oid = await git.resolveRef({ fs, dir: repoPath, ref: `refs/heads/${ref}` })

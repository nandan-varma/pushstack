...
export function getGitBaseUrl(): string {
  ...
  const authUrl = process.env.BETTER_AUTH_URL || process.env.VITE_BETTER_AUTH_URL
  if (authUrl) {
 ...
 */
export function getCloneUrl(owner: string, repoName: string, protocol: 'https' = 'https'): string {
 ...
 */
export function getSetupInstructions(_owner: string, repoName: string, cloneUrl: string) {
  return {
 ...
 */
export function parseGitUrl(url: string): { owner: string; repoName: string } | null {
  try {

    ...
    <div className="space-y-6">
      <Card className="p-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <h2 className="text-2xl font-semibold text-[var(--sea-ink)]">Repository Setup</h2>
            <p className="mt-2 text-sm text-[var(--sea-ink-soft)]">
              Git, R2-backed object storage, and SQL metadata are configured for this repository.
            </p>
          </div>
      ...
      <Card className="p-6">
        <h3 className="text-lg font-semibold text-[var(--sea-ink)]">Clone URL</h3>
        <p className="mt-2 text-sm text-[var(--sea-ink-soft)]">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card className="p-6">
          <h3 className="text-lg font-semibold text-[var(--sea-ink)]">Repository Status</h3>
          <dl className="mt-4 space-y-3 text-sm">
            <div className="flex items-center justify-between gap-4">
              <dt className="text-[var(--sea-ink-soft)]">Visibility</dt>
              <dd className="font-medium text-[var(--sea-ink)]">{repo.visibility}</dd>
            </div>
            <div className="flex items-center justify-between gap-4">
              <dt className="text-[var(--sea-ink-soft)]">Default branch</dt>
              <dd className="font-medium text-[var(--sea-ink)]">{defaultBranch}</dd>
            </div>
            <div className="flex items-center justify-between gap-4">
              <dt className="text-[var(--sea-ink-soft)]">Branches</dt>
              <dd className="font-medium text-[var(--sea-ink)]">{branches?.length || 0}</dd>
            </div>
            <div className="flex items-center justify-between gap-4">
              <dt className="text-[var(--sea-ink-soft)]">Storage</dt>
              <dd className="font-medium text-[var(--sea-ink)]">Git objects in R2</dd>
            </div>
            <div className="flex items-center justify-between gap-4">
        ...
        <Card className="p-6">
          <h3 className="text-lg font-semibold text-[var(--sea-ink)]">Next Actions</h3>
          <div className="mt-4 flex flex-col gap-3">
            <Link to="/repo/$owner/$name/commits" params={{ owner, name }}>
              <Button variant="outline" className="w-full justify-start">View commit history</Button>
            </Link>
            <Link to="/repo/$owner/$name/pulls" params={{ owner, name }}>
              <Button variant="outline" className="w-full justify-start">Open pull requests</Button>
            </Link>
            {isOwner && (
              <Link to="/repo/$owner/$name/upload" params={{ owner, name }}>
                <Button className="w-full justify-start">Add files in the web UI</Button>
              </Link>
            )}
          </div>
        </Card>
      ...
      <Card className="p-6">
        <h3 className="text-lg font-semibold text-[var(--sea-ink)]">Push a New Repository</h3>
        ...
        <Button className="mt-4" variant="outline" onClick={() => handleCopy(instructions.newRepo, 'new')}>
          {copiedSection === 'new' ? 'Copied' : 'Copy commands'}
        </Button>
      </Card>

      <Card className="p-6">
        <h3 className="text-lg font-semibold text-[var(--sea-ink)]">Push an Existing Repository</h3>
        <pre className="mt-4 overflow-x-auto rounded-lg border border-[var(--line)] bg-[var(--chip-bg)] p-4 text-xs text-[var(--sea-ink)]">
